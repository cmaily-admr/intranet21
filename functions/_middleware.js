/**
 * Middleware de protection par mot de passe — PROTECTION AUTOMATIQUE.
 *
 * Principe inversé : au lieu de lister ce qui est protégé, on liste ce qui
 * reste PUBLIC. Tout le reste (toute page, y compris les futures rubriques)
 ...
 *
 * Pour rendre une page publique à l'avenir : ajoutez son chemin à CHEMINS_PUBLICS.
 */

const CHEMINS_PUBLICS = [
  "/",
  "/index.html",
  "/nos-associations.html",
  "/_acces",
];
...
const EXTENSIONS_PUBLIQUES = [
  ".css", ".js", ".png", ".jpg", ".jpeg", ".gif", ".webp", ".svg",
  ".ico", ".woff", ".woff2", ".ttf", ".pdf", ".txt", ".xml", ".json",
  ".mjs", ".map",
];

function estPublic(chemin) {
  if (CHEMINS_PUBLICS.includes(chemin)) return true;
  for (const p of PREFIXES_PUBLICS) {
    if (chemin.startsWith(p)) return true;
  }
  const point = chemin.lastIndexOf(".");
  if (point !== -1) {
    const ext = chemin.slice(point).toLowerCase();
    if (ext !== ".html" && EXTENSIONS_PUBLIQUES.includes(ext)) return true;
  }
  return false;
}
...
<title>Le Kiosque ADMR — Accès réservé</title>
<style>
  ...
  * { box-sizing:border-box; }
  ...
  h1 { margin:0 0 6px; font-size:1.4rem; color:var(--vert-fonce); font-family:Georgia,"Times New Roman",serif; }
...
<body>
  <div class="carte">
    <h1>Le Kiosque ADMR</h1>
    <p class="sous">Ce rayon du Kiosque est réservé aux salariés de l'ADMR de Côte-d'Or. Merci de saisir le mot de passe communiqué par votre association.</p>
    ${msg}
...
export async function onRequest(context) {
  ...
  if (chemin === "/_acces" && request.method === "POST") {
    ...
    if (!destination.startsWith("/") || destination.indexOf("//") !== -1 || estPublic(destination)) {
      destination = "/";
    }
